...
// run on qcif format only for now
  ...
  case 2: { 
    ...
    if (x >= sizeX) { 
      ...
      if (y >= sizeY) { 
        ...
        guchar* eos_rgbbuf2 = (guchar*)((void*)(&rgbbuf2_end));
        ...
        // resample for zoom
        if (zoom == 1)
          memcpy( beg_rgbbuf2, beg_rgbbuf1, sizeX*sizeY*zoom*zoom*3 );
          ...
          for (gint y = 0; y < sizeY*zoom; y+=zoom) {
            for (gint x = 0; x < sizeX*zoom; x+=zoom) {
              for (gint c = 0; c < 3; c++) {
                ...
                default: assert(!"only zoom 1,2,3 allowed");
                }
              }
            }
          }
