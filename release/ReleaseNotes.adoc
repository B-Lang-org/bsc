...
Highlights since release 2021.07:

General
~~~~~~~

* Updates to the install instructions
  ** Show how Bluetcl can be used to programmatically retrieve the BSC
     version
  ** Show how to use Cabal `v2-install`
  ** How to build a release without Asciidoctor

* Support for building and running on more systems

Documentation
~~~~~~~~~~~~~

* Typos fixed in the `MIMO` library documentation
...
* Optional use of DPI instead of VPI, for imported C functions (BDPI)
...
* The Yices submodule is now checked out at an official tagged version,
  2.6.4

* Source updated to compile with GHC 9.2
...
* Fixes to modules in the `Divide` library
  ** for issues when the iterations-per-cycle is greater than one
  ** for scheduling issues at the interface
  ** to improve the provisos

* Fix to `FloatingPoint` divide and square root modules, to not
  require `-aggressive-conditions` flags for correct behavior

* Bug fix in `Prelude` function `hexDigitToInteger`
