...
instance NFData (IRule a) where
    rnf (IRule x1 x2 x3 x4 x5 x6 x7 x8) =
...
instance NFData (IRules a) where
    rnf (IRules x1 x2) = rnf x1 `seq` rnf x2 `seq` ()
...
-- TODO: Incomplete but the original comment mentioned recursion.
instance NFData (IClock a) where
  rnf (IClock x1 x2 _x3) = rnf x1 `seq` rnf x2 `seq` ()
...
instance NFData (IReset a) where
  rnf (IReset x1 _x2 _x3) = rnf x1 `seq` ()
...
instance NFData (IInout a) where
  rnf (IInout x1 x2 _x3) = rnf x1 `seq` rnf x2 `seq` ()
...
-- -- TODO: Broken. Initial comment said that the proper implementation caused cycles somehow.
-- NOTE that using the default NFData instance for Array.Array COULD cause this to loop if there are cycles indeed.
-- instance NFData (ILazyArray a) where
--   rnf _ = ()
...
eSubst v x e = deepseq e' e' -- GEORGE: The heaviest function of the whole lot.
  where e' = sub e
        sub ee@(ILam i t e)
            | v == i = ee
            | i `S.member` fvx =
...
instance NFData (IPackage a) where
    rnf (IPackage x1 x2 x3 x4) = rnf x1 `seq` rnf x2 `seq` rnf x3 `seq` rnf x4 `seq` ()
...
instance NFData (IModule a) where
    rnf (IModule x1 x2 x3 x4 x5 x6 x7 x8 x9 x10 x11 x12 x13 x14 x15 x16) =
        rnf x1 `seq` rnf x2 `seq` rnf x3 `seq` rnf x4 `seq` rnf x5
               `seq` rnf x6 `seq` rnf x7 `seq` rnf x8 `seq` rnf x9
               `seq` rnf x10 `seq` rnf x11 `seq` rnf x12 `seq` rnf x13
               `seq` rnf x14 `seq` rnf x15 `seq` rnf x16 `seq` ()

instance NFData (IEFace a) where
    rnf (IEFace x1 x2 x3 x4 x5 x6) = rnf x1 `seq` rnf x2 `seq` rnf x3 `seq` rnf x4 `seq` rnf x5 `seq` rnf x6 `seq` ()

instance NFData IAbstractInput where
    rnf (IAI_Port x) = rnf x `seq` ()
    rnf (IAI_Clock x1 x2) = rnf x1 `seq` rnf x2 `seq` ()
    rnf (IAI_Reset x) = rnf x `seq` ()
    rnf (IAI_Inout x1 x2) = rnf x1 `seq` rnf x2 `seq` ()

instance NFData (IDef a) where
    rnf (IDef i t e p) = rnf i `seq` rnf t `seq` rnf e `seq` rnf p `seq` ()

instance NFData (IExpr a) where
    rnf (ILam i t e) = rnf i `seq` rnf t `seq` rnf e `seq` ()
    rnf (IAps e ts es) = rnf e `seq` rnf ts `seq` rnf es `seq` ()
    rnf (IVar i) = rnf i `seq` ()
    rnf (ILAM i k e) = rnf i `seq` rnf k `seq` rnf e `seq` ()
    rnf (ICon i ic) = rnf i `seq` rnf ic `seq` ()
    rnf (IRefT t p _) = rnf t `seq` () -- TODO: Broken.

instance NFData (IConInfo a) where
--    rnf (ICDef x1 x2) = rnf x1 `seq` rnf x2 `seq` ()
    rnf ic@(ICDef x1 x2) = ()                        -- XXX a hack to avoid circular defs
    rnf (ICPrim x1 x2) = rnf x1 `seq` rnf x2 `seq` ()
    rnf (ICForeign x1 x2 x3 x4 x5) = rnf x1 `seq` rnf x2 `seq` rnf x3 `seq` rnf x4 `seq` rnf x5 `seq` ()
    rnf (ICCon x1 x2) = rnf x1 `seq` rnf x2 `seq` ()
    rnf (ICIs x1 x2) = rnf x1 `seq` rnf x2 `seq` ()
    rnf (ICOut x1 x2) = rnf x1 `seq` rnf x2 `seq` ()
    rnf (ICTuple x1 x2) = rnf x1 `seq` rnf x2 `seq` ()
    rnf (ICSel x1 x2 x3) = rnf x1 `seq` rnf x2 `seq` rnf x3 `seq` ()
    rnf (ICVerilog x1 x2 x3 x4) = rnf x1 `seq` rnf x2 `seq` rnf x3 `seq` rnf x4 `seq` ()
    rnf (ICUndet x1 x2 x3) = rnf x1 `seq` rnf x2 `seq` rnf x3 `seq` ()
    rnf (ICInt x1 x2) = rnf x1 `seq` rnf x2 `seq` ()
    rnf (ICReal x1 x2) = rnf x1 `seq` rnf x2 `seq` ()
    rnf (ICString x1 x2) = rnf x1 `seq` rnf x2 `seq` ()
    rnf (ICChar x1 x2) = rnf x1 `seq` rnf x2 `seq` ()
    rnf (ICHandle x1 x2) = rnf x1 `seq` rnf x2 `seq` ()
    rnf (ICStateVar x1 x2) = rnf x1 `seq` rnf x2 `seq` ()
    rnf (ICMethArg x1) = rnf x1 `seq` ()
    rnf (ICModPort x1) = rnf x1 `seq` ()
    rnf (ICModParam x1) = rnf x1 `seq` ()
--    rnf (ICValue x1 x2 x3) = rnf x1 `seq` rnf x2 `seq` rnf x3 `seq` ()        -- XXX causes cycles somehow
    rnf (ICValue _x1 _x2) = ()
    ...
    rnf (ICName x1 x2) = rnf x1 `seq` rnf x2 `seq` ()
    rnf (ICAttrib x1 x2) = rnf x1 `seq` rnf x2 `seq` ()
    -- TODO: does not force the lazy array bit
    rnf (ICLazyArray x1 _x2 x3) = rnf x1 `seq` rnf x3 `seq` ()
    rnf (ICPosition x1 x2) = rnf x1 `seq` rnf x2 `seq` ()
    rnf (ICType x1 x2) = rnf x1 `seq` rnf x2 `seq` ()
    rnf (ICPred x1 x2) = rnf x1 `seq` rnf x2 `seq` ()

instance NFData (IStateVar a) where
    -- TODO: Also broken.
    rnf IStateVar {isv_uid = x} = rnf x `seq` ()
