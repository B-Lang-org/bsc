...
import TopUtils(putStrLnF, getClockSuffix)
...
dumpConflictMapDOT errh flags prefix modId
    ...
    let modName = getIdBaseString modId
        graph = conflictMapToDOT suffix modName
                                 ifcRuleNames userRuleNames cmap
    dumpDOTGraph errh flags prefix modName "conflict" graph


conflictMapToDOT :: String -> String -> [AId] -> [AId] -> ConflictMap ->
                    DOTGraph
conflictMapToDOT clockSuffix modName ifcRuleNames userRuleNames cmap =
    let
...
dumpExecGraphDOT errh flags prefix modId
    ...
    suffix <- getClockSuffix flags
    let modName = getIdBaseString modId
        graph = execGraphToDOT suffix modName
                               ifcRuleNames userRuleNames scgraph
    dumpDOTGraph errh flags prefix modName "exec" graph


execGraphToDOT :: String -> String -> [AId] -> [AId] -> [(AId,[AId])] ->
                  DOTGraph
execGraphToDOT clockSuffix modName ifcRuleNames userRuleNames scgraph =
    let
        node_stmts = [ Node (getIdString i) [NShape NBox]
                       | i <- ifcRuleNames ] ++
                     [ Node (getIdString i) [NShape NEllipse]
...
dumpUrgencyGraphDOT errh flags prefix modId
    ...
    suffix <- getClockSuffix flags
    let modName = getIdBaseString modId
        graph = urgencyGraphToDOT suffix modName
                                  ifcRuleNames userRuleNames umap
    dumpDOTGraph errh flags prefix modName "urgency" graph


urgencyGraphToDOT :: String -> String -> [AId] -> [AId] -> UrgencyMap ->
                     DOTGraph
urgencyGraphToDOT clockSuffix modName ifcRuleNames userRuleNames umap =
    let
        node_stmts = [ Node (getIdString i) [NShape NBox]
                       | i <- ifcRuleNames ] ++
                     [ Node (getIdString i) [NShape NEllipse]
...
dumpCombinedGraphDOT errh flags prefix modId
    ...
    suffix <- getClockSuffix flags
    let modName = getIdBaseString modId
        graph = combinedGraphToDOT suffix modName
                                   ifcRuleNames userRuleNames seq_graph
    dumpDOTGraph errh flags prefix modName "combined" graph


combinedGraphToDOT :: String -> String -> [AId] -> [AId] ->
                      [(CSNode,[CSNode])] ->
                      DOTGraph
combinedGraphToDOT clockSuffix modName ifcRuleNames userRuleNames seq_graph =
    let
...
dumpCombinedGraphFullDOT errh flags prefix modId
    ...
    suffix <- getClockSuffix flags
    let modName = getIdBaseString modId
        graph = combinedGraphFullToDOT suffix modName
                                       ifcRuleNames userRuleNames seq_map
    dumpDOTGraph errh flags prefix modName "combined_full" graph


combinedGraphFullToDOT :: String -> String -> [AId] -> [AId] -> CSMap ->
                          DOTGraph
combinedGraphFullToDOT clockSuffix modName ifcRuleNames userRuleNames seq_map =
    let
