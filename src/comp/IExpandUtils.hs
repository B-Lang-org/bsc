...
fullTypeNormalizer flags symt (ITAp f@(ITCon op _ _) a)
  | op == idSizeOf && canNorm a' = normalizeSizeOf $ ITAp f a'
  where a' = fullTypeNormalizer flags symt a
        ...
        normalizeSizeOf t =
          ...
          in case t' of
               ITNum _ -> t'
               _ -> internalError $ "fullTypeNormalizer - unsimplified: " ++ ppReadable (t,t')
