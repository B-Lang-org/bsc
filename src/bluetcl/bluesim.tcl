...
if {$show_version != 0} {
    ...
    if {[lindex $vinfo 2] == ""} then {
        set version [join [lrange $vinfo 0 1] "."]
    } else {
        set version [join [lrange $vinfo 0 2] "."]
    }
    set rev [lindex $vinfo 3]
    set date [clock format [lindex $vinfo 4]]
    puts "Model generated by bsc $version (build $rev)"
