...
// This doesn't count the 1-bits, just strips
// off the trailing 1-bit four times. If it's not zero,
// then there are >4 1-bits.
static inline unsigned Dar_FiveOrMoreBits( unsigned uWord )
{
   uWord = (uWord-1)&uWord;
   uWord = (uWord-1)&uWord;
   uWord = (uWord-1)&uWord;
  ...
  return uWord; // non-zero if >=5 ones.
}
...
Dar_Cut_t * Dar_ObjPrepareCuts( Dar_Man_t * p, Aig_Obj_t * pObj )
{
    ...
    // Replaced code to remove a spurious Valgrind warning.
...
Dar_Cut_t * Dar_ObjComputeCuts( Dar_Man_t * p, Aig_Obj_t * pObj )
{
    ...
    Dar_ObjForEachCut( pFaninR1, pCut1, k )
    {
        ...
        if (0 != Dar_FiveOrMoreBits(pCut0->uSign | pCut1->uSign))
          continue;
