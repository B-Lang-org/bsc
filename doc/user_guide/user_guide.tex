...
\index{-stable-verilog@\te{-stable-verilog} (compiler flag)}
\begin{centerboxverbatim}
...
-stable-verilog         deterministic Verilog output (canonical names
                         and ordering)
\end{centerboxverbatim}
...
The {\bf\tt -stable-verilog} flag makes the generated Verilog a pure
function of the elaborated design: a module regenerates byte-identically
(for example, from its \te{.ba} file with the \te{-c} codegen mode)
regardless of what else was compiled in the same invocation or of the
compilation history.  The names of compiler-introduced signals (for
example \te{x\_\_h5} or \te{\_\_d7} suffixes) and the ordering of
declarations and assignments no longer depend on compiler-internal
state.  This flag is \emph{on} by default; \te{-no-stable-verilog}
restores the historical behavior, in which those names and orderings
could differ between otherwise-identical compiles.
