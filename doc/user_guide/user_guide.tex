...
Models built with {\bf\tt -dump-formats fst} additionally require
{\bf\tt -lz}, for the FST waveform writer.
...
+bscvcd[=<file>] = dump waveforms to VCD file (default: dump.vcd)
+bscfst[=<file>] = dump waveforms to FST file (default: dump.fst)
\end{centerboxverbatim}
...
The {\bf\tt +bscvcd} and {\bf\tt +bscfst} plus-args select waveform
dumping in VCD or FST format, respectively, uniformly with the
Verilog simulators.  A file name can be given with
{\bf\tt +bscvcd=<file>} or {\bf\tt +bscfst=<file>}; the defaults are
``dump.vcd'' and ``dump.fst''.  The requested format must have been
compiled into the simulation model with the {\bf\tt -dump-formats}
flag of BSC (VCD is available by default); otherwise the simulator
reports an error.  Like other plus-args, these values are also
visible to the model via {\bf\tt \$test\$plusargs}.
...
\index{Bluesim!scripting!commands!fst}
\subsubsubsection{fst}

The {\bf\tt sim fst} command controls dumping of waveforms to an FST
file, in the same way that {\bf\tt sim vcd} controls dumping to a VCD
file.  Use {\bf\tt sim fst on} to enable dumping and
{\bf\tt sim fst off} to disable it.  The form {\bf\tt sim fst <file>}
enables dumping to a file of the given name, rather than the default
file named ``{\tt dump.fst}''.  The simulation model must have been
built with {\bf\tt -dump-formats fst}.
...
Bluesim can also record the same waveforms in GTKWave's compressed
binary FST format, which is typically much smaller and faster to load
in a viewer than VCD.  FST support is compiled into a simulation
model by giving the {\bf\tt -dump-formats fst} flag (or
{\bf\tt -dump-formats vcd,fst} for both formats) when linking with
{\bf\tt -sim}; at run time the format is selected with the
{\bf\tt +bscfst[=<file>]} plus-arg or the {\bf\tt sim fst} command.
FSDB files are not generated by Bluesim.
