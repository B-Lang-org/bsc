...
& \\ \cline{2-2}
&\begin{libverbatim}
function Vector#(n_chunk, chunk_type) toChunks(a_type x)
    provisos( Bits#(chunk_type, chunk_sz), Bits#(a_type, a_sz)
            , Div#(a_sz, chunk_sz, n_chunk) );
\end{libverbatim}
\\
\hline
\end{tabular}
...
\index[function]{Vector!fromChunks}
\begin{tabular}{|p{.9 in}|p{4.7 in}|}
\hline
&\\ \te{fromChunks}&The inverse of \te{toChunks}. Convert a Vector of
chunks to a value, dropping any padding on the final chunk. \\
& \\ \cline{2-2}
&\begin{libverbatim}
function atype fromChunks(Vector#(n_chunk, chunk_type) v)
    provisos( Bits#(chunk_type, chunk_sz), Bits#(a_type, a_sz)
            , Div#(a_sz, chunk_sz, n_chunk) );
\end{libverbatim}
\\
